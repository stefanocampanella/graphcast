...
from graphcast.dataset_utils import (
  DateIntervalsRange,
  Process,
  ProvidersRegistry,
  check_coordinates,
...
def _parse_timeseries_arguments(configs, output_path, start=None, end=None, array_id=None):
  ...
  if start is None or end is None:
    raise ValueError(
      "Start and end dates must be specified when downloading timeseries, either in the config file "
  ...
  if array_id is not None:
    ...
    if array_config and step is not None:
      ...
      if array_id >= len(intervals_range):
        raise IndexError(
          f"Invalid array ID {array_id}. Must be between 0 and {len(intervals_range) - 1}."
        )
      start = intervals_range[array_id].start
      end = (
        intervals_range[array_id + 1].start
        if array_id + 1 < len(intervals_range)
        else intervals_range.end
      )
      output_path = output_path / f"{start.strftime('%Y%m%d')}-{end.strftime('%Y%m%d')}"
    else:
      raise ValueError(
        "Configs top table should contain the 'slurm_array' table specifying the 'step' key when "
...
def download(
  ...
  if dataset_type == "static":
    ...
    date_intervals, output_path = _parse_timeseries_arguments(
      configs, output_path, start=start, end=end, array_id=array_id
    )
...
def slurm_array_range(
  ...
  while True:
    try:
      date_interval, _ = _parse_timeseries_arguments(
...
@click.argument(
  "data_dir", required=True, type=click.Path(path_type=pathlib.Path, dir_okay=True, readable=True)
)
@click.argument(
  "output_path",
  required=True,
...
def merge(
  ...
  for dataset_conf in configs.get("datasets", []):
    ...
    (
      datasets.append(
        dataset_reader(data_dir / dataset_conf["file"], **dataset_conf.get("kwargs", {}))
      ),
    )
...
def normalization(
  ...
  boolean_variables = set(
    var for (var, data) in dataset.data_vars.items() if np.isdtype(data.dtype, np.bool_)
  )
  ...
  template_dataset = (
    dataset[list(missing_normalization_variables)]
    if missing_normalization_variables
    else xr.Dataset()
  )
...
def unpack(
  ...
  save_to_zarr(
    ...
    output_path,
    overwrite=overwrite,
    compressor_kwargs=dict(cname=cname, clevel=clevel),
  )
