...
class LossFunction(Protocol):
  ...
  def __call__(
    self, predictions: xr.Dataset, targets: xr.Dataset, **optional_kwargs
  ) -> LossAndDiagnostics:
...
def weighted_mse(
  ...
  weighted_mse_fn = get_weighted_loss(
    squared_error,
...
def weighted_tweedie_deviance(
  ...
  weighted_tweedie_deviance_fn = get_weighted_loss(
    tweedie_deviance,
    per_variable_weights=per_variable_weights,
    levels_normalization_coord=levels_normalization_coord,
    weights_decreasing_with_level=weights_decreasing_with_level,
...
def get_weighted_loss(
  loss_fn: Callable[[xr.DataArray, xr.DataArray], xr.DataArray],
  per_variable_weights: Mapping[str, float] | None = None,
  levels_normalization_coord: str = "level",
  weights_decreasing_with_level: bool = False,
  mask: xr.DataArray | None = None,
  ...
  def weighted_loss(predictions: xr.Dataset, targets: xr.Dataset) -> LossAndDiagnostics:
    loss_per_variable_fn = get_weighted_loss_per_variable(
      loss_fn,
      levels_normalization_coord=levels_normalization_coord,
      weights_decreasing_with_level=weights_decreasing_with_level,
      mask=mask,
...
def get_weighted_loss_per_variable(
  loss_fn: Callable[[xr.DataArray, xr.DataArray], xr.DataArray],
  levels_normalization_coord: str = "level",
  weights_decreasing_with_level: bool = False,
  mask: xr.DataArray | None = None,
  ...
  def weighted_loss_per_variable_fn(prediction: xr.DataArray, target: xr.DataArray) -> xr.DataArray:
    ...
    if "level" in target.dims:
      loss = loss * normalized_level_weights(
        target, coord=levels_normalization_coord, decreasing=weights_decreasing_with_level
      ).astype(loss.dtype)
...
def sum_per_variable_losses(
  ...
  total = xr.concat(weighted_per_variable_losses.values(), dim="variable", join="exact").sum(
    dim="variable", skipna=False
  )
  return total, per_variable_losses


def normalized_level_weights(
  data: xr.DataArray, coord: str = "level", w_min=1e-2, decreasing: bool = False
) -> xr.DataArray:
