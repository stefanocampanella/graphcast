...
class Mask(predictor_base.Predictor):
  ...
  def __call__(
    self, inputs: xr.Dataset, targets_template: xr.Dataset, forcings: xr.Dataset, **kwargs
  ) -> xr.Dataset:
    inputs = inputs.fillna(value=self._value)
    forcings = forcings.fillna(value=self._value)
    predictions = self._predictor(
      inputs=inputs, targets_template=targets_template, forcings=forcings, **kwargs
    )
    return predictions.where(self._mask, np.nan)

  def loss(
    self, inputs: xr.Dataset, targets: xr.Dataset, forcings: xr.Dataset, **kwargs
  ) -> predictor_base.LossAndDiagnostics:
    loss_and_diagnostics, _ = self.loss_and_predictions(
      inputs=inputs, targets=targets, forcings=forcings, **kwargs
    )
  ...
  def loss_and_predictions(
    ...
    loss_and_diagnostics, predictions = self._predictor.loss_and_predictions(
      inputs=inputs, targets=targets, forcings=forcings, mask=self._mask, **kwargs
    )
