...
from graphcast import checkpoint, icosahedral_mesh, model
from graphcast.mesh_graph import MeshGraph, faces_to_edges, merge_meshes
...
def upgrade_legacy_model_config(
  old_model_config: ModelConfig,
  grid_mask: xarray.DataArray,
  grid_weights: xarray.DataArray | None = None,
) -> model.ModelConfig:
  ...
  attributes_to_copy = [
    name for name in old_model_config.__dict__ if name in model.ModelConfig.__dataclass_fields__
  ]
...
def read_legacy_checkpoint(
  checkpoint_file_path: PathLike, mask_and_weights_file_path: PathLike
) -> model.ModelConfig:
