...
from grain.checkpoint import CheckpointRestore as IterDatasetRestore
from grain.checkpoint import CheckpointSave as IterDatasetSave
from grain.experimental import pick_performance_config
from grain.python import DatasetIterator, IterDataset
...
from graphcast.model import CheckPoint, GraphCast, ModelConfig, TaskConfig
...
MaybeDatasets = tuple[Datasets | None, ...]
DatasetsOrDataArrays = tuple[xr.Dataset | xr.DataArray, ...]
...
def get_artifacts(data_path: epath.Path, configs: Configs) -> MaybeDatasets:
  ...
  return tuple(
    _get_ds(name) for name in ["mean_by_level", "stddev_by_level", "diffs_stddev_by_level"]
  )
...
def get_model_config(configs: Configs) -> ModelConfig:
  return ModelConfig(
    ...
    radius_query_fraction_edge_length=configs.get(
      "model.radius_query_fraction_edge_length", required=True
    ),
...
def get_dataset_iterator(
  ...
  if local_batch_size > 1:
    ...
    def batch_fn(samples):
      return tuple(
        map(lambda datasets: xr.concat(datasets, dim="batch"), zip(*samples, strict=False))
      )

    dataset = dataset.batch(batch_size=local_batch_size, drop_remainder=True, batch_fn=batch_fn)
  ...
  if pick_config := configs.get("dataset.pick_performance_config", None):
    ...
    read_options = grain.ReadOptions(
      num_threads=read_config.get("num_threads", required=True),
  ...
  if mp_options is not None:
    ...
    # SharedMemoryArrays.
    dataset_iterator = (
      dataset.map(lambda value: xarray_jax.wrap_data(value, to_jax=False, np_contiguous=True))
...
def get_first_sample_and_reset(
  dataset_iterator: DatasetIterator[InputsTargetsForcings],
) -> InputsTargetsForcings:
...
def get_checkpoint_manager(
  ...
  ckpt_mngr_options = ocp.CheckpointManagerOptions(
    best_fn=lambda metrics: metrics[0], best_mode="min", **configs.get("checkpoints", {})
  )
...
def next_batches_on_device(
  ...
  if mp_prefetch:
    try:
      ...
      batches = xarray_jax.make_array_from_process_local_data(
        batches_on_host, mesh=device_mesh, spec=P("batch")
      )
    ...
    batches_on_host = next(train_iterator), next(test_iterator)
    batch, batch_test = xarray_jax.make_array_from_process_local_data(
      batches_on_host, mesh=device_mesh, spec=P("batch")
    )
...
class TensorboardLogger:
  def __init__(self, tb_path: pathlib.Path):
    ...
    if not jax.distributed.is_initialized():
      raise ValueError(
        "JAX distributed should be initialized beforehand, "
  ...
  def log(
    self,
    current_step: int,
    train_metrics: JAXLossAndDiagnostics,
    test_metrics: JAXLossAndDiagnostics,
    lr: float | None = None,
  ) -> None:
...
def save_model(
  ...
  device_mesh = jax.make_mesh(
    (jax.device_count(), jax.local_device_count()), ("process", "local_device")
  )
...
def fsdp_map(local_fn, mesh: jax.sharding.Mesh, batch_dim_name: str = "batch"):
  ...
  def _pmean_grad_fn(params, rng_key, data, static_data):
    return jax.lax.pmean(
      _local_grad_fn(params, rng_key, data, static_data), axis_name=batch_dim_name
    )

  def global_fn(params, rng_key, data, static_data):
    _global_grad_fn = shard_map(
      _pmean_grad_fn,
      mesh=mesh,
...
def reshard_data(dataset, sharding, datetime_coord_name="time"):
  ...
  dataset = dataset.map(
    lambda da: jax.tree_util.tree_map(
      lambda xs: jax.make_array_from_process_local_data(sharding=sharding, local_data=xs), da
    )
  )
